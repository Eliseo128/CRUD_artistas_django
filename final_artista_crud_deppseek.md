# Proyecto Django para Gestión de Artistas y Canciones (Sin forms.py)

Voy a guiarte paso a paso en la creación de un proyecto Django completo para gestionar artistas y canciones, pero esta vez sin utilizar un archivo forms.py, manejando los formularios directamente en las plantillas HTML.
...
### 6. Crear las vistas (sin forms.py)
...
def crear_artista(request):
    """Vista para crear un nuevo artista (sin forms.py)"""
    if request.method == 'POST':
        # Procesar el formulario enviado
        nombre = request.POST.get('nombre')
        nacionalidad = request.POST.get('nacionalidad')
        foto_artista = request.FILES.get('foto_artista')
        
        # Crear y guardar el nuevo artista
        artista = Artista(
            nombre=nombre,
            nacionalidad=nacionalidad,
            foto_artista=foto_artista
        )
        artista.save()
    ...
    return render(request, 'formulario_artista.html', {'titulo': 'Crear Artista', 'artista': None})

def editar_artista(request, artista_id):
    """Vista para editar un artista existente (sin forms.py)"""
    artista = get_object_or_404(Artista, id=artista_id)
    
    if request.method == 'POST':
        # Procesar el formulario enviado
        artista.nombre = request.POST.get('nombre')
        artista.nacionalidad = request.POST.get('nacionalidad')
        
        # Solo actualizar la foto si se proporciona una nueva
        if request.FILES.get('foto_artista'):
            artista.foto_artista = request.FILES.get('foto_artista')
        
        artista.save()
        return redirect('detalle_artista', artista_id=artista.id)
    
    # Si es GET, mostrar formulario con datos existentes
    return render(request, 'formulario_artista.html', {'titulo': 'Editar Artista', 'artista': artista})
    ...
    <form method="post" enctype="multipart/form-data" class="artist-form">
        ...
        <div class="form-group">
            ...
            <input type="text" id="nombre" name="nombre" class="form-control" 
                   value="{{ artista.nombre|default:'' }}" required>
        </div>
        
        <div class="form-group">
            <label for="nacionalidad">Nacionalidad:</label>
            <input type="text" id="nacionalidad" name="nacionalidad" class="form-control" 
                   value="{{ artista.nacionalidad|default:'' }}" required>
        </div>
        
        <div class="form-group">
            <label for="foto_artista">Foto:</label>
            <input type="file" id="foto_artista" name="foto_artista" class="form-control">
            
            {% if artista and artista.foto_artista %}
            <div class="current-image">
                <p>Imagen actual:</p>
                <img src="{{ artista.foto_artista.url }}" alt="Imagen actual" class="current-img">
            </div>
            {% endif %}
        </div>
...
<div class="container">
    ...
    <div class="confirmation-box">
        <p>¿Estás seguro de que deseas eliminar al artista <strong>{{ artista.nombre }}</strong>?</p>
        ...
        <form method="post">
            ...
            <div class="form-actions">
                ...
                <a href="{% url 'detalle_artista' artista.id %}" class="btn-cancel">Cancelar</a>
            </div>
        </form>
    </div>
</div>
...
### 10. Realizar migraciones y ejecutar el servidor
...
Este proyecto implementa un sistema CRUD completo para gestionar artistas y sus canciones sin utilizar forms.py. Incluye:

1. **Modelos**: Artista y Canción con relación uno a muchos
2. **Vistas**: Listar, detalle, crear, editar y eliminar artistas
3. **Formularios**: Manipulados directamente en las plantillas HTML
...
- El proyecto no utiliza forms.py, manejando los formularios directamente en las vistas y plantillas
- Se incluye un archivo de confirmación para la eliminación de artistas
- El código está comentado para facilitar la comprensión
- Los estilos CSS hacen que la aplicación sea visualmente atractiva
- La aplicación es totalmente funcional y lista para usar
